...
import com.neokred.pos.common.http.RetrofitClient;
import com.neokred.pos.common.http.api.RequestOnlineAuthAPI;
import com.neokred.pos.common.http.model.AuthRequest;
...
//        stopLoading();
    ...
    public BindingCommand cancleTxnsCommand = new BindingCommand(() -> {
        startLoading("processing...");
        new Thread(() -> {
            NeokredPOSService.getInstance(getApplication()).cancelTrade();
        }).start();
    });
    ...
    public void requestOnlineAuth(boolean isICC, PaymentModel paymentModel) {
        ...
        addSubscribe(apiService.sendMessage(AUTHFROMISSUER_URL, authRequest).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(response -> {
            ...
            if (response.isOk()) {
                if (isICC) {
                    NeokredPOSService.getInstance(getApplication()).sendOnlineProcessResult("8A02" + onlineRspCode);
                }else {
                    isOnlineSuccess.setValue(true);
                }
            } else {
                if (isICC) {
                    NeokredPOSService.getInstance(getApplication()).sendOnlineProcessResult("8A023030");
                }else {
                    isOnlineSuccess.setValue(true);
                }
            }
