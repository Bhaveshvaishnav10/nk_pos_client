...
import com.neokred.pos.BuildConfig;
...
public class TerminalApplication extends BaseApplication {
    ...
    public void onCreate() {
        super.onCreate();
        initCrash();
        // Commented out Bugly crash reporting
        // initBugly();
        // Commented out Shiply auto-update system
        // initShiply();
        // Initialize Fragment Cache
        FragmentCacheManager.getInstance();
        TRACE.setContext(this);
        NeokredPOSService.getInstance(this);

        // Initialize UART connection for smart devices (D60, D30, D50, D80) at startup
        // This matches the official demo app pattern and ensures card reader hardware is ready
        if (DeviceUtils.isSmartDevices()) {
            TRACE.i("📱 Smart device detected: " + DeviceUtils.getPhoneModel());
            TRACE.i("🔌 Initializing UART connection at app startup...");
            new Thread(() -> {
                try {
                    // Small delay to ensure app context is fully initialized
                    Thread.sleep(500);
                    NeokredPOSService.getInstance(TerminalApplication.this).openUart();
                    TRACE.i("✅ UART initialization complete at startup");
                } catch (Exception e) {
                    TRACE.e("❌ Failed to initialize UART at startup: " + e.getMessage());
                }
            }).start();
        }
    }
    ...
    private void initCrash() {
        CaocConfig.Builder.create()
                ...
                .errorDrawable(R.mipmap.ic_neokred_logo) //error icon
