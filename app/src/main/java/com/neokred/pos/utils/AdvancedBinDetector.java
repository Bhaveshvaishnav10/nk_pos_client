...
public class AdvancedBinDetector {
    ...
    public static CardType detectCardType(String cardNumber) {
        ...
        if (isRuPay(bin2, bin4, bin6)) return CardType.RUPAY;
     ...
     * RuPay检测
     */
    private static boolean isRuPay(String bin2, String bin4, String bin6) {
        // RuPay BIN ranges: 607, 608, 6521, 6522, 817, 8181, 353, 508, 982

        // Check 4-digit BIN first (more specific)
        if ("8181".equals(bin4)) {
            return true;
        }

        // Check 3-digit BIN patterns
        if (bin6.startsWith("607") || bin6.startsWith("608")) {
            return true;
        }
        if (bin6.startsWith("6521") || bin6.startsWith("6522")) {
            return true;
        }
        if (bin6.startsWith("817") || bin6.startsWith("353")) {
            return true;
        }
        if (bin6.startsWith("508") || bin6.startsWith("982")) {
            return true;
        }
        return false;
    }
